...
public class Appointment {
    ...
    public Appointment(String date, int doctorId, String notes, DBProxy dbProxy){
        ...
        String query = "INSERT INTO appointment (date, Pid, Did, Notes) VALUES (" + this.date.toString() + ", " + this.patientId + ", " + this.doctorId + ", " + this.notes +")";
...
//    }
    ...
    public Appointment[] ReadDoctorApps(int doctorId, DBProxy dbProxy) {
        ...
        try (ResultSet resultSet = dbProxy.executeQuery(appointmentQuery)) {
            ...
            int appointmentCount = resultSet.getRow(); // Get the row number (total rows)
            resultSet.beforeFirst(); // Reset cursor to the beginning

            // Initialize the array
            Appointment[] appointmentsArray = new Appointment[appointmentCount];

            // Iterate through the ResultSet and populate the array
            int i = 0;
            while (resultSet.next()) {
                appointmentsArray[i++] = new Appointment(
